...
def create_summary_endpoint(summary: SummaryCreate, db: Session = Depends(get_db)):
    return create_summary(db=db, summary=summary)


@router.get("/", response_model=List[SummaryOut], status_code=status.HTTP_200_OK)
def read_summaries(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return get_all_summaries(db, skip=skip, limit=limit)
...
def read_summary(summary_id: int, db: Session = Depends(get_db)):
    summary = get_summary(db, summary_id=summary_id)
    if not summary:
...
def delete_summary_endpoint(summary_id: int, db: Session = Depends(get_db)):
    summary = get_summary(db, summary_id=summary_id)
    if not summary:
        raise HTTPException(status_code=404, detail="Resumen no encontrado")
    return delete_summary(db, summary_id=summary_id)


@router.get("/notebook/{notebook_id}", response_model=List[SummaryOut], status_code=status.HTTP_200_OK)
def read_summaries_by_notebook(notebook_id: int, db: Session = Depends(get_db)):
    summaries = get_summaries_by_notebook(db, notebook_id=notebook_id)
    if not summaries:
...
def read_summaries_by_user(user_id: int, db: Session = Depends(get_db)):
    summaries = get_summaries_by_user(db, user_id=user_id)
    if not summaries:
