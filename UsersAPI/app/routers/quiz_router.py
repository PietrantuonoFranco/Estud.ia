...
def read_quizzes(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return get_all_quizzes(db, skip=skip, limit=limit)
...
def delete_quiz_endpoint(quiz_id: int, db: Session = Depends(get_db)):
    ...
    if not quiz:
        raise HTTPException(status_code=404, detail="Quiz no encontrado")
    return delete_quiz(db, quiz_id=quiz_id)


@router.get("/notebook/{notebook_id}", response_model=List[QuizOut], status_code=status.HTTP_200_OK)
def read_quizzes_by_notebook(notebook_id: int, db: Session = Depends(get_db)):
    quizzes = get_quizzes_by_notebook(db, notebook_id=notebook_id)
    if not quizzes:
...
def read_quizzes_by_user(user_id: int, db: Session = Depends(get_db)):
    quizzes = get_quizzes_by_user(db, user_id=user_id)
    if not quizzes:
...
def create_question_endpoint(quiz_id: int, question: QuestionCreate, db: Session = Depends(get_db)):
    if not get_quiz(db, quiz_id=quiz_id):
        raise HTTPException(status_code=404, detail="Quiz no encontrado")
    return create_question(db, question, quiz_id=quiz_id)
