...
class FlashcardGraphState(TypedDict):
    """
    FlashcardGraph state for flashcards
    """
...
class FlashcardGraph:
    """
    Grafo para la creación de flashcards basado en un PDF
    """
    ...
    async def retrieve_context(self, state: FlashcardGraphState) -> FlashcardGraphState:
        """
    ...
    async def generate_flashcards(self, state: FlashcardGraphState) -> FlashcardGraphState:
        """
        Genera automáticamente 5 preguntas con sus respuestas para flashcards.
        """
...
Responde únicamente con el JSON:""")
    ...
    def build(self):
        ...
        """
        self.workflow = StateGraph(FlashcardGraphState)
        
        # Agregar nodos (ahora son métodos de la clase)
        self.workflow.add_node("retrieve", self.retrieve_context)
        self.workflow.add_node("generate", self.generate_flashcards)
    ...
    async def invoke(self, initial_state: FlashcardGraphState):
        """
...
def create_flashcard_graph(embedding_generator: EmbeddingGenerator, client_milvus:Async_Milvus_Client):
    """
    Factory function que crea una instancia de FlashcardGraph y construye el grafo
    
    Args:
        embedding_generator: Generador de embeddings
        client_milvus: Cliente asíncrono de Milvus
    """
    flashcard_graph = FlashcardGraph(embedding_generator, client_milvus)
    flashcard_graph.build()
